...
def get_picture():
    all_photo_list = tool.read("SELECT place_id,photo,ispet FROM myapp_attractions")
    for photo in all_photo_list:
        if not photo[2] : continue
        if check_file_in_folder(f'{os.getcwd()}\\寵物\\',f'{photo[0]}_0.jpg'): continue
        ...
        # # 将照片保存到文件
        with open(f"{os.getcwd()}\\寵物\\{photo[0]}_0.jpg", "wb") as file:
