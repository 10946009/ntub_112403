...
with open("臺北市bar.csv", newline="", encoding="utf-8") as csvfile:
    ...
    for address in address_list[1:]:
        ...
        # try:
        with open(f"{address[0]}{address[1]}酒吧.json", encoding="utf-8") as file:
            data = json.load(file)

        with open(f"{address[0]}{address[1]}酒吧擁擠資訊.json", encoding="utf-8") as file:
            data_crowd = json.load(file)

        with open(f"{address[0]}{address[1]}酒吧營業時間.json", encoding="utf-8") as file:
