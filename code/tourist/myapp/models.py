...
class Crowd(models.Model):
    ...
    crowd = ArrayField(models.IntegerField(), blank=True)
    opening = ArrayField(models.TextField(), blank=True)
...
class Comment(models.Model):
    h_id = models.ForeignKey(to=History, on_delete=models.CASCADE)#分享沒了留言也會被刪除
    a_id = models.ForeignKey(to=Attractions, on_delete=models.CASCADE)#景點沒了留言也會被刪除
    u_id = models.ForeignKey(to=User, on_delete=models.CASCADE)#景點沒了留言也會被刪除
    name = models.TextField(max_length=500)
    content = models.TextField(max_length=500)
    score = models.FloatField(null=False, blank=False)
    time = models.TimeField(auto_now_add=True)
    comment_type =  models.IntegerField(null=False, blank=False)
